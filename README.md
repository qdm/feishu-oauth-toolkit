...
# 3. Install cloudflared and start the tunnel + callback server
...
# 4. Get the public URL and paste it into the Feishu backend
tunnel-url   # prints e.g. https://xyz.trycloudflare.com

# 5. Generate the authorization URL
feishu-auth-url   # prints full URL; open in browser, approve, watch the callback land

# 6. Run the end-to-end verification
feishu-verify
```
...
See [docs/feishu-backend-setup.md](docs/feishu-backend-setup.md) for the exact backend steps required before you can use these scopes.
